...
using DistributedSystem.Infrastructor.Caching;
using DistributeSystem.Application.Abstractions;
using DistributeSystem.Contract.JsonConverters;
using DistributeSystem.Infrastructure.Authenication;
using DistributeSystem.Infrastructure.BackgroubJobs;
using DistributeSystem.Infrastructure.DependencyInjection.Options;
using DistributeSystem.Infrastructure.PipeObservers;
using MassTransit;
...
namespace DistributeSystem.Infrastructure.DependencyInjection.Extensions
{
    public static class SeviceCollectionExtentions
    {
        ...
        public static void AddQuartzInfratructure(this IServiceCollection services)
        {
                ...
                .AddTrigger(
                    trigger =>
                        trigger.ForJob(jobKey)
                            .WithSimpleSchedule(
                                schedule =>
                                    schedule.WithInterval(TimeSpan.FromMilliseconds(1))
                                    .RepeatForever()));
